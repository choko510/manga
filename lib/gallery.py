...
from .uri import get_nozomi_uri, async_get_nozomi_uri
from .utility import (
    HitomiError,
    binary_search,
    async_binary_search,
    async_fetch,
    async_get_node_at_address,
    fetch,
    get_id_set,
    get_node_at_address,
)
...
async def async_get_gallery(gallery_id: int) -> Gallery:
    response = await async_fetch(f"{RESOURCE_DOMAIN}/galleries/{gallery_id}.js")
    response_text = response.decode("utf-8")
    json_start = response_text.find("{")
    if json_start == -1:
        raise HitomiError(ErrorCode.INVALID_VALUE, "response", "contain valid JSON")
...
async def async_get_gallery_ids(options: Optional[dict] = None) -> List[int]:
    options = options or {}
    title = options.get("title")
    tags: Optional[List[Tag]] = options.get("tags")
    range_options: Optional[dict] = options.get("range")
    ...
    version = (await async_fetch(f"{RESOURCE_DOMAIN}/galleriesindex/version")).decode("utf-8")
    id_sets: List[IdSet] = []
    ...
    if popularity is not None or is_range_available or (is_tags_available and getattr(tags[0], "is_negative", False)):
        ...
        if popularity is not None:
            nozomi_options["popularityOrderBy"] = popularity
        uri = await async_get_nozomi_uri(nozomi_options)
        headers = None
        if is_range_available and not should_slice_result and range_options is not None:
            start = range_options.get("start")
            end = range_options.get("end")
            ...
            end_bytes = f"{end * 4 - 1}" if isinstance(end, int) else ""
            headers = {"Range": f"bytes={start_bytes}-{end_bytes}"}
        id_sets.append(get_id_set(await async_fetch(uri, headers=headers)))

    if is_title_available:
        processed_title = title.lower() + " "
        current_index = 0
        next_index = processed_title.find(" ")
        root_node = await async_get_node_at_address(0, version)
        if root_node is None:
            raise HitomiError(ErrorCode.LACK_OF_ELEMENT, "galleriesIndex")
        while next_index != -1:
            if next_index - current_index == 0:
            ...
            word = processed_title[current_index:next_index]
            key = hashlib.sha256(word.encode("utf-8")).digest()[:4]
            data = await async_binary_search(key, root_node, version)
            if data is not None:
                address, length = data
    ...
    if is_tags_available and tags is not None:
        for tag in tags:
            uri = await async_get_nozomi_uri({"tag": tag})
            id_sets.append(get_id_set(await async_fetch(uri), is_negative=getattr(tag, "is_negative", False)))

    base_set = get_id_set(await async_fetch(await async_get_nozomi_uri()))
    for extra in id_sets:
        base_set = _combine_id_sets(base_set, extra)
